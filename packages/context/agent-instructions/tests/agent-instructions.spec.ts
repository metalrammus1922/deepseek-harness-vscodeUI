...
describe('workspace context instruction discovery', () => {
  ...
  it('defaults dshHome and uses cwd itself as root when no project marker exists', async () => {
    ...
    // the home dirs point at an empty dir so the default ~/.dsh-vscodeui holds no global
    // scope. Windows homedir() reads USERPROFILE (not HOME), so both must be
    // stubbed or a real ~/.dsh-vscodeui/AGENTS.md would otherwise leak in.
    vi.stubEnv('DSH_HOME', '')
  ...
  it('labels the default DSH home as ~/.dsh-vscodeui when HOME points at the configured default', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      ...
      expect(files.map(file => file.displayPath)).toEqual(['~/.dsh-vscodeui/AGENTS.md'])
    } finally {
      vi.unstubAllEnvs()
      vi.doUnmock('node:os')
      vi.resetModules()
  ...
  it('expands a configured ~/.dsh-vscodeui home to the operating-system home directory', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      ...
      vi.doMock('node:os', () => ({ homedir: () => home }))
      const isolated = await import('@deepseek-ai/dsh-agent-instructions')
      const files = await isolated.discoverBaselineInstructionFiles({ cwd: root, dshHome: '~/.dsh-vscodeui' })

      expect(files).toEqual([{ absolutePath: join(home, '.dsh/AGENTS.md'), displayPath: '~/.dsh-vscodeui/AGENTS.md' }])
    } finally {
      vi.doUnmock('node:os')
      vi.resetModules()
      await rm(root, { recursive: true, force: true })
...
describe('workspace context request injection', () => {
  ...
  it('labels a custom dshHome as DSH_HOME instead of pretending it is ~/.dsh-vscodeui', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
