...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  return {
    ...
    git: {
      // Read-only repository inspection for the browser's source-control
      // surface. The cwd is caller-supplied (the session workspace root);
      // absent it, the host process working directory is inspected.
      async status(request, signal) {
        try {
          const cwd = request.payload.cwd ?? process.cwd()
          return ok(request, await gitStatus(cwd, signal))
        } catch (error: unknown) {
          if (signal.aborted) {
