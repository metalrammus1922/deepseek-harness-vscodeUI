...
`resolveDshHome()` resolves the single-root DeepSeek Harness home. Precedence, highest first: an explicit configured path, `$DSH_HOME`, then `~/.dsh-vscodeui`. The harness keeps all user data under one root.
...
`dshHomeDisplay()` names an active root symbolically for user-facing paths: `~/.dsh-vscodeui` for the default home, `$DSH_HOME` for any configured home. It never leaks an absolute machine path.
